...
- **Назначение `sipId`**: `bucis` ведёт **отображение `mac -> sipId`** в памяти процесса и при повторном `ClientQuery` для того же MAC возвращает **тот же** `sipId` (см. 11.4).
...
В предыдущем контуре у проекта были отдельные пакеты `internal/control/scheduler/` и `internal/session/`, но в текущем дереве кода они отсутствуют (и не импортируются).
Нужная для EC функциональность уже реализована в других местах:

- **Планирование “по абсолютному \(t0\)”**: `internal/media/sender/sender.go` (`StreamAt`, `StreamFramesAt`) — старт и стрим RTP-кадров строго по реальному времени от заданного \(t0\), с обработкой “пропусков” при GC/scheduler stall.
- **Управление жизненным циклом медиа-сессии**: `internal/media/rtpsession/rtpsession.go` (`Start`) — оркестрация receiver/playback/capture/sender, единый `cleanup()`, остановка по `ctx.Done()`.
- **Хранение состояния и таймеры EC/SIP**: `internal/app/bes/*` и `internal/app/bucis/*` — FSM/состояния и таймауты относятся к прикладной логике ролей, а не к отдельному “универсальному session-пакету”.
...
### 1.1.4. RTP пакетизация, G.726 и RTP-статистика (loss/jitter)
...
- считать потери и джиттер (RTP-диагностика),
...
## 2.1. Архитектурные слои симулятора (нормативно для реализации)

Чтобы не смешивать control/signaling/media и чтобы закрывать edge-cases без “if-ов и флагов”, реализация должна быть организована слоями:

```
EC layer (UDP, wire-format)
   ↓ (события/команды EC)
Session layer (sipId, mac, timers, FSM, корреляция)
   ↓ (команды на SIP/media + события от них)
SIP layer (REGISTER/INVITE/BYE, SDP)
   ↓
RTP/media layer (RTP/G.726, audio sink/source)
```

Нормативные требования к границам:

- EC-слой — **только** парсинг/формирование пакетов и доставка/приём по UDP, без бизнес-переходов.
- Session-слой — **единственный источник истины** по состоянию (FSM), таймерам, `sipId`/MAC-корреляции и политике повторов/таймаутов.
- SIP/RTP слои — транспортные/медийные детали; в unit-тестах допускается их заменять “фейком” (без реальных сокетов/OpenSIPS/ALSA), но интеграционный прогон должен использовать реальные UDP+SIP+RTP.
...
- Для EC-симулятора фиксируем адресацию “как в поезде”, но с удобными dev-дефолтами:
  - `ClientReset` и `KeepAlive`:
    - **интеграционно**: broadcast (например, `192.168.5.255:8890`), т.к. БУЦИС не знает адреса всех БЭС до первого `ClientQuery`;
    - **dev по умолчанию**: unicast на `EC_BES_ADDR` (например, `127.0.0.1:8890`), т.к. broadcast часто не работает в окружениях разработки.
...
- **Dual-port правило**: `bucis` принимает `ClientQuery` на `6710/7777`, но ответ `ClientAnswer` отправляет **всегда** на `src_ip(ClientQuery):8890` (вне зависимости от входного порта запроса).
...
- **ALSA в CI/без звука**: режим без аудио должен быть **dev-дефолтом** (`ALSA_DEVICE=null` или `NO_AUDIO=1`). RTP при этом принимается, G.726 декодируется, вывод — в `null` sink; RTP-статистика (lost/jitter) продолжается.
...
### 8.5.6.1. Windows 11: нативный звук без WSL

На Windows роль `bes` может работать со звуком **без WSL2**, но вместо ALSA используются внешние утилиты **ffmpeg/ffplay**:

- **воспроизведение**: `ffplay` читает raw PCM `S16LE, 8 kHz, mono` из stdin
- **захват микрофона**: `ffmpeg -f dshow` пишет тот же raw PCM в stdout

Это сделано намеренно, чтобы сборка `*.exe` оставалась **без cgo**.

Настройка:

- установите ffmpeg (чтобы в `PATH` были `ffmpeg.exe` и `ffplay.exe`)
- на Windows задайте `ALSA_DEVICE` как **имя микрофона DirectShow**

Как узнать имя микрофона (PowerShell/cmd):
...
```
ALSA_DEVICE=Microphone (Realtek(R) Audio)
```
...
- **интеграционно (как в поезде)** используем **broadcast** для пакетов “БУЦИС → все БЭС”:
  - `ClientReset` → `192.168.5.255:8890`
  - `KeepAlive` → `192.168.5.255:8890`
- **dev по умолчанию** используем **unicast** для `ClientReset/KeepAlive` (например, `EC_BES_ADDR=127.0.0.1`), т.к. broadcast в dev-сетях часто недоступен/неочевиден.
...
Важно: можно переиспользовать **механизм** конфигурации из `internal/infra/config` (env + flags), но legacy-ключи старых контуров (`CONTROL_*`, `MEDIA_PORT`, `AUDIO_FILE` и т.п.) не считаются частью целевого интерфейса EC-симулятора и должны быть заменены на EC/SIP/RTP ключи выше.
