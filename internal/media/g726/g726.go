...
func quantAdapt(c *g726Core, d int) int {
	...
	if i == 0 {
		// После `i = ^i` при sign≠0 получается −1; далее индекс маскируется до 4 бит (см. inverseQuant).
		// Нужен код 15 (все единицы в младших 4 битах), а не 0 — иначе шаг квантования схлопнется.
		i = 15
	}
	return i
}
