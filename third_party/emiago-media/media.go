...
// TakeRTPConn extracts the underlying UDP connection from the session and nils
// the internal reference so that a subsequent Close() does not double-close it.
// The caller becomes responsible for closing the returned conn.
// Returns nil if the internal conn is not a *net.UDPConn or is already nil.
func (s *MediaSession) TakeRTPConn() *net.UDPConn {
	c, _ := s.rtpConn.(*net.UDPConn)
	if c != nil {
		s.rtpConn = nil
	}
	return c
